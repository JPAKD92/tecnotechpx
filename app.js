...
/* ---- Logo PNG real (poné un archivo "logo.png" en la raíz del repo) ----
   Si el archivo existe, se usa en el encabezado y en el PDF.
   Si no existe, se usa el logo vectorial de arriba como respaldo. */
const LOGO_FILE = "logo.png";
const logoImg = new Image();
let logoReady = false, logoRatio = 260 / 78;
logoImg.onload = () => {
  logoReady = true;
  if (logoImg.naturalWidth) logoRatio = logoImg.naturalWidth / logoImg.naturalHeight;
  renderHeaderLogo();
};
logoImg.onerror = () => { logoReady = false; renderHeaderLogo(); };
logoImg.src = LOGO_FILE;

function renderHeaderLogo() {
  const el = document.getElementById("brandLogo");
  if (!el) return;
  el.innerHTML = logoReady
    ? `<img src="${LOGO_FILE}" alt="TECNOTECH DIÉSEL" style="height:42px;display:block">`
    : LOGO_SVG;
}
...
function generatePDF(returnDoc) {
  ...
  // logo: imagen real (logo.png) si está disponible; si no, logo vectorial
  function drawHeaderTextLogo() {
  ...
  if (logoReady && logoImg.complete) {
    try {
      const lh = 19, lw = Math.min(lh * logoRatio, 95);
      doc.addImage(logoImg, "PNG", M, 5.5, lw, lh);
      logoBottom = 5.5 + lh;
    } catch (e) { drawHeaderTextLogo(); }
  } else {
    drawHeaderTextLogo();
  }
  doc.setTextColor(180, 190, 210); doc.setFont("helvetica", "normal"); doc.setFontSize(8);
  doc.text("Tel: " + COMPANY.phone, M, Math.max(27, logoBottom + 3.5));
...
async function shareWhatsApp() {
  ...
  // Construir el PDF como archivo (sin descargarlo)
  let doc, file = null;
  try {
    doc = generatePDF(true);
  ...
  // 1) Compartir el ARCHIVO directo (celular con Web Share API).
  //    Elegimos el formato aceptado ANTES de compartir, y llamamos a share()
  //    UNA sola vez (clave para iPhone: no perder el permiso de usuario).
  if (file && navigator.share) {
    const candidates = [
      { files: [file], title: msg, text: msg },
      { files: [file], text: msg },
      { files: [file] }
    ];
    let chosen = null;
    for (const p of candidates) {
      if (!navigator.canShare || navigator.canShare(p)) { chosen = p; break; }
    }
    if (chosen) {
      try {
        await navigator.share(chosen);
        return; // PDF compartido/adjuntado con éxito
      } catch (e) {
        if (e && e.name === "AbortError") return; // el usuario canceló
        // otro error -> usa el fallback de abajo
      }
    }
  }

  // 2) Fallback (PC o navegador sin compartir archivos): descargar + abrir WhatsApp con texto
  doc.save(fname);
  window.open("https://wa.me/?text=" + encodeURIComponent(msg), "_blank");
  toast("📎 Tu navegador no permite adjuntar el PDF solo. Lo descargué para que lo adjuntes.");
}
